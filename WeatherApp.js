...
const WeatherApp = (lat,lon,location) =>{
    return new Promise((resolve,reject)=>{
        axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&appid=fde56c4e5d999bb74d63aa4c2675d76c`).then((response)=>{
            resolve({weather : `${location} current temparature is ${response.data.main.temp} C and ${response.data.weather[0].description} situation.` })
